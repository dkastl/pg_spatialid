...
- `bbox` (`geometry`): The 3D bounding box (PolygonZ). Must include all spatial extents and can be created in any supported CRS (coordinate reference system). The bounding box is typically created by `ST_Envelope` of two 3D points.
...
- The `bbox` must be a valid 3D geometry (PolygonZ). The SRID and units must be managed by the user as appropriate for their use case.
- Use `bbox::box3d AS box3d` to convert the geometry to a 3D box for easier handling in queries.
